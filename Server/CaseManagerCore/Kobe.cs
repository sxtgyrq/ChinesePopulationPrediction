...
namespace CaseManagerCore
{
    public class Kobe : CaseManager
    {
        ...
        // string filePath = System.Configuration.ConfigurationManager.AppSettings["ProblemPath"] + "KebeJingshen.xml";// System.Configuration. @"Xmlfolder\";
        static string filePath = "";                                                                                                    //string HashCode = "";
        ...
        public Kobe()
        {
            ...
            {
                var myJsonString = File.ReadAllText("Identitycard.json");

                var obj = Newtonsoft.Json.JsonConvert.DeserializeObject<config>(myJsonString);
                Kobe.filePath = obj.dic + "KebeJingshen.xml";
            }
            this.idPrevious = "qhy";
            ...
            {
                ObjType = "html",
                msg = $"老师您好，现在是{DateTime.Now.ToString("yyyy年MM月dd日 HH:mm")}。我给您讲讲科比和黑曼巴精神，你看看对不对，不对您要及时指出我的错误哦！关注要瑞卿@知乎，了解更多。",
                showContinue = true,
                showIsError = false,
                isEnd = false,
            ...
            int newMax;
            if (int.TryParse(xmlDoc.ChildNodes[2].Attributes["newMax"].Value, out newMax))
            {
            ...
            var newOrderGet = new List<int>();
            for (int i = 0; i < Math.Min(orderGet.Count, newMax); i++)
            {
                newOrderGet.Add(orderGet[i]);
            }
            orderGet = newOrderGet;
            xmlDoc = null;
        }
        ...
        {
            MysqlCore.BaseItem b = new MysqlCore.BaseItem("kobe");
